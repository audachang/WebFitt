...
//added to match FittsTrace format
var groupCode = "G00";
var taskType = "2D";
var selectionMethod = "MB"; //Mouse-button (MB), Dwell time (DT), keyboard (KB)
var interactionMethod = "PS"; //point-select (PS), Drag-select (DS), Fitts farm (FF)
...
let traceDataHeader = ["App","Participant","Condition","Session","Group","TaskType","SelectionMethod","InteractionMethod","Block","Sequence","A","W","Trial","from_x","from_y","to_x","to_y","{t_x_y}"];
...
function runPipeline() {
    ...
    if (clickNumber == n + 1) {
        if (taskIdx < tasks.length - 1) {
            ...
            //var filename = "FittsTask-" + participantCode + "-" + conditionCode + "-" + sessionCode + "-G00-2D-MB-PS-B01";
            var filename = "FittsTask-" + participantCode + "-" + conditionCode + "-" + sessionCode;
...
function onCanvasClick() {
    ...
    if (correct && !beginFlag) {
        ...
        var src = getTargetPosition(uncalibratedTasks[taskIdx].A, n, getTargetIdxFromClickNumber(clickNumber, n));
        var tgt = getTargetPosition(uncalibratedTasks[taskIdx].A, n, getTargetIdxFromClickNumber(clickNumber + 1, n));
        startTrace(uncalibratedTasks[taskIdx].A, uncalibratedTasks[taskIdx].W, taskIdx, clickNumber, src, tgt);
        clickNumber++;
    }
    else if (beginFlag) {
        currentClickTime = millis();
        computeClickData(clickPos);
        endTrace(currentClickTime);
        clickNumber++;
        lastClickTime = currentClickTime;
        if (clickNumber <= n) {
            var src2 = getTargetPosition(uncalibratedTasks[taskIdx].A, n, getTargetIdxFromClickNumber(clickNumber - 1, n));
            var tgt2 = getTargetPosition(uncalibratedTasks[taskIdx].A, n, getTargetIdxFromClickNumber(clickNumber, n));
            startTrace(uncalibratedTasks[taskIdx].A, uncalibratedTasks[taskIdx].W, taskIdx, clickNumber - 1, src2, tgt2);
        }
    }
}
...
// Initializes a new trace after a click
function startTrace(A, W, block, trial, fromPos, toPos) {
    currentTrace = {
        A: A,
        W: W,
        block: block,
        ...
        startTime: millis(),
        times: [0],
        xs: [Math.round(mouseX)],
        ys: [Math.round(mouseY)]
    };
}

// Records mouse position for the current trace
function recordTrace() {
    if (currentTrace) {
        var t = millis() - currentTrace.startTime;
        currentTrace.times.push(Math.round(t));
        currentTrace.xs.push(Math.round(mouseX));
        currentTrace.ys.push(Math.round(mouseY));
    }
}

// Finalizes the current trace
function endTrace(timestamp) {
    if (currentTrace) {
        currentTrace.times.push(Math.round(timestamp - currentTrace.startTime));
        currentTrace.xs.push(Math.round(mouseX));
        currentTrace.ys.push(Math.round(mouseY));
        traceData.push(currentTrace);
        currentTrace = null;
    }
}
...
// Generates trace result string
function generateTraceResultString(){
    var resultString = "TRACE DATA\n" + traceDataHeader.join(",") + "\n";
    for(var i = 0; i < traceData.length; i++){
        ...
        var base = [
            ...
            "MB",
            "PS",
            "B" + padNumber(tr.block + 1, 2),
            0,
            tr.A,
            tr.W,
            tr.trial,
            Math.round(tr.from_x),
            Math.round(tr.from_y),
            Math.round(tr.to_x),
            Math.round(tr.to_y)
        ].join(",");
        resultString += base + ",t=," + tr.times.join(",") + ",\n";
        resultString += base + ",x=," + tr.xs.join(",") + ",\n";
        resultString += base + ",y=," + tr.ys.join(",") + ",\n";
    }
    return resultString;
}
...
function uploadResult() {
    ...
    var filename = "WebFitts-" + participantCode + "-" + conditionCode + "-" + sessionCode + "-" + pointingDevice;
...
function saveAsZipFile(filename) {
        ...
        zip.file(filename + ".sd3", generateTraceResultString());
